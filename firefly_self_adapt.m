function [GBestmem,GBestval,nfeval1,Gmin,Giter,GSure] = firefly_self_adapt(fname,VTR,D,XVmin,XVmax,NP,itermax,refresh,tekrar,Alpha,Gamma,Delta)
...
% self-adaptive structure for adjusting parameters of alpha and gamma...
%-----------------------------------------------------------------
        katsayi = rand(1,4);
        if katsayi(2) < tho_1,
           alpha = alpha_low + katsayi(1)*alpha_up; 
        end
        if katsayi(4) < tho_2,
           gamma = gamma_low + katsayi(3)*gamma_up; 
        end
%----------------------------------------------------------------- 
