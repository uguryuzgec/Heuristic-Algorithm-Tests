...
for r=1:tekrar
fprintf(1,'%d. tekrar\n',r);
pop = zeros(NP,D*2); %initialize pop to gain speed
...
%%pop(i,:) = rand(1,D*2);
    for k=1:D,
        pop(i,k*2-1:k*2) = XVmin(k) + rand(1,D).*(XVmax(k) - XVmin(k));
        Distance(i,k)=sqrt(pop(i,k*2-1)^2+pop(i,k*2)^2);
		S(i,k)=1/Distance(i,k);
    end
end

%%%if Smell() eger sinirlari gecerse ne olacak??? 
  for i=1:NP
      for j=1:D,
         if(S(i,j)<XVmin(j))
            S(i,j)=XVmin(j);
        elseif(S(i,j)>XVmax(j))
            S(i,j)=XVmax(j);
        end
      end
  end
...
popold(i,:)=pop(i,:)+2*rand(1,D*2)-1;

    for k=1:D,
         %pop(i,k*2-1:k*2) = XVmin(k) + rand(1,D).*(XVmax(k) - XVmin(k));
         %popold(i,:)=pop(i,:)+2*rand(1,D*2)-1;
         Distance(i,k)=sqrt(popold(i,k*2-1)^2+popold(i,k*2)^2);
